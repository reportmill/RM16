...
import reportmill.util.RMKeyChain.Op;
...
public class RMKeyChainAggr {
    ...
    public static Object getValue(Object aRoot, List aList, RMKeyChain aKeyChain)
    {
        ...
        // Handle FunctionCall: if aggregate method, return result of aggregate with args (eval tail too, if present)
            ...
            if (args.getChildCount()==0)
                args = null;
            else if (args.getChildCount()==1)
                args = args.getChildKeyChain(0);
    ...
    {
        Method meth = _methods.get(aName);
        if (meth==null)
            _methods.put(aName, meth=getAggrMethodImpl(aName));
        return meth!=_emptyMeth ? meth : null;
    }
    ...
    public static double total(List aList, RMKeyChain aKeyChain)
    {
        ...
        if (shouldRecurse(aList, aKeyChain)) {
            for (int i=0, iMax=aList.size(); i<iMax; i++) {
                List sublist = (List) aList.get(i);
                Double value = total(sublist, aKeyChain);
                total += value;
            }
        }
        ...
        // If shouldn't recurse, sum result of evaluating keychain on objects in list
            ...
            double val = RMKeyChain.getDoubleValue(aList.get(i), aKeyChain);
            total += val;
        }
    ...
    public static Double totalX(List aList, RMKeyChain aKeyChain)
    {
        ...
        if (shouldRecurse(aList, aKeyChain)) {
            for (int i=0, iMax=aList.size(); i<iMax; i++) {
                List sublist = (List) aList.get(i);
                Double val = totalX(sublist, aKeyChain);
                if (val==null)
                    return null;
                total += val;
            }
        ...
        // If shouldn't recurse, sum result of evaluating keychain on objects in list
            ...
            Number val = (Number) RMKeyChain.getValue(aList.get(i), aKeyChain);
            if (val==null)
                return null;
    ...
    private static boolean hasAggregate(RMKeyChain aKC)
    {
        ...
        if (aKC.getOp()==Op.Key || aKC.getOp()==Op.FunctionCall) {
            ...
            Method meth = RMKeyChainAggr.getAggrMethod(key);
            if (meth != null)
                return true;
        }
        ...
        for (int i=0, iMax=aKC.getChildCount(); i<iMax; i++) {
            ...
            if (child instanceof RMKeyChain && hasAggregate((RMKeyChain)child))
                return true;
        }
