...
import reportmill.util.DataUtils;
   ...
   double            _prefHeight;
    ...
    public SGView rpgAll()
    {
        ...
        if (_prefHeight>0) {
            ...
            if (getChildCount()>0)
                _prefHeight = getChildLast().getFrameMaxY();
        }
        ...
        for (RMTableRPG pg=rshape; pg!=null; pg=pg._nextPage)
            slist.addChild(pg);
        return slist;
    }
    ...
    protected boolean addRows(RMGroup aGroup, RMTableRowRPG aParentRPG, RMTableRowRPG theLastRow)
    {
        ...
        if (headerRow!=null && (!aGroup.isEmpty() || headerRow.getPrintEvenIfGroupIsEmpty())) {
            ...
            boolean didAddRow = addRow(rowRPG, aParentRPG);
            if (!didAddRow)
                return false;
            aParentRPG = rowRPG;
        }
        ...
        for (int i=0, iMax=aGroup.size(); i<iMax; i++,added++) { RMGroup childGroup = aGroup.getGroup(i);
            ...
            if (!childGroup.isLeaf()) {
                boolean didAddChildGroupRows = addRows(childGroup, parentRPG, theLastRow);
                if (!didAddChildGroupRows)
                    break;
            }

            // If child table exists, recurse (for RMTableGroupRPG)
            else {
                boolean didAddChildTableRows = !addChildTableRows(childGroup, parentRPG, theLastRow);
                if (!didAddChildTableRows)
                    break;
            }
        ...
        if (added<aGroup.size()) {
            ...
            if (summaryRow==null || !summaryRow.hasVersion("Running"))
                return false;
            ...
            while (getChildCount()>pageStartRowIndex) {
                ...
                removeRow(_lastRow);
                while (_lastRow.isSummary())
                    removeRow(_lastRow=(RMTableRowRPG)getChildLast());
                while (!isSatisfied(_lastRow._parentRPG, -1))
                    removeRow(_lastRow=(RMTableRowRPG)getChildLast());
            }
    ...
    boolean addRow(RMTableRowRPG aRow, RMTableRowRPG aParentRPG)
    {
        ...
        if (aRow.isSummary()) {
            removeRow(aRow);
            if (aRow._row.getNumberOfChildrenToStayWith()==0)
                return false;
            if (aParentRPG.getChildRPGCount()>0)
                _lastRow = (RMTableRowRPG)getChildLast();
            else return false;
        }

        // Iterate up parentRPGs while unsatisfied constraints
        RMTableRowRPG lastRow = _lastRow;
        RMTableRowRPG parentRPG = lastRow._parentRPG;
    ...
    {
        for (int i=aRow.getChildRPGCount()-1; i>=0; i--)
            removeRow(aRow._childRPGs.get(i));
    ...
    boolean splitRow(RMTableRowRPG aRow)
    {
        ...
        RMTableRowRPG split = (RMTableRowRPG)aRow.divideViewFromTop(part1Height);
        aRow._split = split;
        split.setBestHeight();
        split.layoutDeep();
        return true;
