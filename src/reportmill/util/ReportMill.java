...
public class ReportMill {
    ...
    {
        if (appServer!=null)
            return appServer.convertFromAppServerType(anObj); // If AppServer, have it do conversion
        if (anObj instanceof Set)
            return new ArrayList((Set)anObj); // If Set, return List
        if (anObj instanceof Object[])
            return Arrays.asList((Object[])anObj); // If Array, return List
    ...
    {
        return Prefs.getPrefs(ReportMill.class);
    }
    ...
    public static String getBuildInfo()
    {
        ...
        try { _buildInfo = SnapUtils.getText(SnapUtils.class, "/rmdraw/BuildInfo.txt").trim(); }
    ...
    public static void init()
    {
        ...
        String rver = StringUtils.toString(getVersion());
        String info = String.format("Build Date: %s, Version %s, JVM %s, User %s", bdate, rver, jver, user);
