...
public class RMCrossTab extends RMParentShape {
    ...
    protected XMLElement toXMLShape(RMArchiver anArchiver)
    {
        ...
        for (int i = 0, iMax = getRowCount(); i < iMax; i++)
            for (int j = 0, jMax = getColCount(); j < jMax; j++) {
                RMCrossTabCell cell = getCell(i, j);
                if (cell.getRow() != i || cell.getCol() != j) continue;
                XMLElement cxml = anArchiver.writeObjectToXml(cell);
                cxml.add("row", i);
                cxml.add("col", j);
                e.add(cxml);
            }
    ...
    protected void fromXMLShape(RMArchiver anArchiver, XMLElement anElement)
    {
        ...
        for (int i = anElement.indexOf("column"); i >= 0; i = anElement.indexOf("column", i + 1)) {
            ...
            RMCrossTabCol col = anArchiver.readObjectFromXmlForClass(cxml, RMCrossTabCol.class);
            _cols.add(col);
            col._table = this;
        }
        ...
        for (int i = anElement.indexOf("row"), ri = 0; i >= 0; i = anElement.indexOf("row", i + 1), ri++) {
            ...
            RMCrossTabRow row = anArchiver.readObjectFromXmlForClass(rxml, RMCrossTabRow.class);
            _rows.add(ri, row);
            row._table = this;
            ...
            for (int j = rxml.indexOf("cell"), ci = 0; j >= 0; j = rxml.indexOf("cell", j + 1)) {
                ...
                RMCrossTabCell cell = anArchiver.readObjectFromXmlForClass(cellX, RMCrossTabCell.class);
        ...
        for (int i = anElement.indexOf("cell"); i >= 0; i = anElement.indexOf("cell", i + 1)) {
            XMLElement cxml = anElement.get(i);
            RMCrossTabCell cell = anArchiver.readObjectFromXmlForClass(cxml, RMCrossTabCell.class);
