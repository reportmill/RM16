...
public class ReportOwner implements RMKeyChain.Get {
    ...
    public void addModelObject(Object anObj)
    {
        ...
        if (obj.getClass().getName().equals("java.sql.ResultSet"))
        ...
        else if (obj instanceof Map) { Map map = (Map)obj;
        ...
        if (_model.size()>0 && (_dataStack.size()==0 || _dataStack.get(0)!=_model))
            _dataStack.add(0, _model);
    }
            ...
            if (valString != null) {
                ...
                if (valString instanceof String) { String string = (String)valString;
                    ...
                    if (StringUtils.startsWithIC(string, "<html"))
                        valString = RMEnv.getEnv().parseHTML(string, keyRun.getFont(), keyLine.getLineStyle());
